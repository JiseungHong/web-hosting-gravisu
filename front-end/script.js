...
function function3(e) {
  ...
  fetch("http://110.76.86.172:8000/run-gradcam", {
    ...
    .then((data) => {
      ...
      alert(`GRAVISU processing completed in ${data.duration}`);
      ...
      updateImageDisplay(data.image_paths, data.histogram);

      runButton.textContent = "Run GradCam++";
      runButton.disabled = false;
      submitButton1.disabled = false;
      submitButton2.disabled = false;
    })
...
function updateImageDisplay(imagePaths, histogram) {
  ...
  if (chartElement) {
    chartElement.src = baseUrl + histogram;
